...
- `Gitea/openapi.json` — API schema, download of `https://git.schindlerfelix.de/openapi3.v1.json` plus two local content-type fixes applied by `./fetch-spec.sh` (the only committed source of truth; the plugin accepts JSON natively). **Always regenerate via `./fetch-spec.sh`, never raw curl** — the server spec declares `application/octet-stream` for `repoGetRawFile` and `application/json` for plain-string responses (diffs/patches, markup, gpg token), but the server actually serves `text/plain`, which makes the generated client throw `unexpectedContentTypeHeader` at runtime. The script re-applies both fixes; they're documented in the script itself.
